...
- (instancetype)initWithTitle:(NSString *)title items:(NSArray *)items
{
    ...
    if ( self )
    {
        ...
        self.buttonView = [[UIScrollView alloc]init];
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i )
        {
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.width.equalTo(self.buttonView);
                if ( !firstButton )
                {
                    firstButton = btn;
        ...
        if([items count]>6){
             scrollHeight=config.buttonHeight*6;
            [_buttonView setScrollEnabled:YES];
        }else{
            scrollHeight=config.buttonHeight*[items count];
            [_buttonView setScrollEnabled:NO];
        }
        [_buttonView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.height.offset(scrollHeight);
        }];;
