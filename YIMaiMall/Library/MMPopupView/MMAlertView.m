...
- (instancetype)initWithTitle:(NSString *)title contentArray:(NSArray *)array items:(NSArray *)items
{
    self = [super init];
    ...
    if ( self )
    {
        ...
        if ( title.length > 0 )
        {
            ...
            self.titleLabel.textColor = RGB(110, 110, 110);
...
//            PayWay * way = array[i];
            ...
            [ctrl mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.height.mas_equalTo([MyAdapter laDapter:40]);
            }];
            [ctrl addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
            
            //            //imageView
            UIImageView * imgView = [[UIImageView alloc] init];
            [ctrl addSubview:imgView];
            [imgView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(ctrl).offset(0);
                make.left.equalTo(self).offset(config.innerMargin);
                make.height.mas_equalTo([MyAdapter laDapter:40]);
                make.width.mas_equalTo([MyAdapter laDapter:40]);
            }];
            imgView.contentMode = UIViewContentModeCenter;
//            UIImage * image = [[UIImage imageNamed:way.payImage] scaledToSize:CGSizeMake([MyAdapter laDapter:27], [MyAdapter laDapter:27])];
//            imgView.image = image;
            ...
            [label mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(ctrl).offset(0);
                make.left.equalTo(imgView.mas_right).offset(config.innerMargin);
                make.height.mas_equalTo([MyAdapter laDapter:40]);
            }];
            label.textColor = RGB(130, 130, 130);
//            label.text = way.payName;
            label.numberOfLines = 0;
            //button
            ImageBtn * btn = [[ImageBtn alloc] init];
            btn.tag = 1000+i;
            [ctrl addSubview:btn];
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(ctrl).offset(0);
                make.left.equalTo(label.mas_right).offset(config.innerMargin);
                make.right.equalTo(self).offset(-config.innerMargin);
                make.height.mas_equalTo([MyAdapter laDapter:40]);
                make.width.mas_equalTo([MyAdapter laDapter:40]);
            }];
            btn.adjustsImageWhenHighlighted = NO;
//            UIImage * normalImage = [[UIImage imageNamed:@"dot_agree"] scaledToSize:CGSizeMake(20, 20)];
//            [btn setImage:normalImage forState:UIControlStateDisabled];
//            UIImage * selectImage = [[UIImage imageNamed:@"dot_notagree"] scaledToSize:CGSizeMake(20, 20)];
//            [btn setImage:selectImage forState:UIControlStateNormal];
            if (i == 0) {
                btn.enabled = NO;
            }
            lastAttribute = ctrl.mas_bottom;
        }
        
        self.buttonView = [UIView new];
        [self addSubview:self.buttonView];
        [self.buttonView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(lastAttribute).offset(config.innerMargin * 0.5);
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i )
        {
            ...
            [btn setTitleColor:item.highlight?config.itemHighlightColor:RGB(130, 130, 130) forState:UIControlStateNormal];
...
- (void)actionButton:(UIButton*)btn
{
    ...
    else
    {
        ...
        if (item.handler1) {
            for (UIView * v in self.subviews) {
                if ([v isKindOfClass:[UIControl class]]) {
                    for (UIView * vv in v.subviews) {
                        if ([vv isKindOfClass:[ImageBtn class]]) {
                            ImageBtn * btn = (ImageBtn *)vv;
                            if (btn.enabled == NO) {
            ...
            item.handler1(btn.tag,self.selectIndex);
        }
    }
}
...
- (void)buttonClick:(UIControl *)ctrl{
...
//        self.itemHighlightColor = kAppThemeColor;//;MMHexColor(0xE76153FF);
