...
static float const kPopViewCellHeight = 40.f; ///< cell指定高度
static float const kPopoverViewArrowHeight = 13.f; ///< 箭头高度

// convert degrees to radians
float PopDegreesToRadians(float angle) {
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<CustomPopAction *> *actions;
...
- (void)setStyle:(PopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [PopViewCell bottomLineColorForStyle:_style];
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, cornerRadius + maskTop)
                        radius:cornerRadius
                    startAngle:PopDegreesToRadians(180)
                      endAngle:PopDegreesToRadians(270)
                     clockwise:YES];
    // 箭头向上时的箭头位置
    if (_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:PopDegreesToRadians(270)
                      endAngle:PopDegreesToRadians(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:PopDegreesToRadians(0)
                      endAngle:PopDegreesToRadians(90)
                     clockwise:YES];
    // 箭头向下时的箭头位置
    if (!_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:PopDegreesToRadians(90)
                      endAngle:PopDegreesToRadians(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
...
- (CGFloat)calculateMaxWidth {
    CGFloat maxWidth = 0.f, titleLeftEdge = 0.f, imageWidth = 0.f, imageMaxHeight = kPopViewCellHeight - PopoverViewCellVerticalMargin*2;
    CGSize imageSize = CGSizeZero;
    UIFont *titleFont = [PopViewCell titleFont];
    for (CustomPopAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<CustomPopAction *> *)actions {
...
/*! @brief 指向指定的点来显示弹窗 */
- (void)showToPoint:(CGPoint)toPoint withActions:(NSArray<CustomPopAction *> *)actions {
    _actions = [actions copy];
    // 计算箭头指向方向
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    PopViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[PopViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        CustomPopAction *action = _actions[indexPath.row];
