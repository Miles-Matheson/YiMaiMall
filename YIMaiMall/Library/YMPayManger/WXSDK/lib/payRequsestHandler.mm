...
- ( NSMutableDictionary *)sendPay_demo:(Order*)order
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = order.productName;
    //订单金额,单位（分）
    NSString *order_price   =order.amount;//1分钱测试
    ...
    NSString *orderno   = order.tradeNO;
    ...
    [packageParams setObject: order.notifyURL        forKey:@"notify_url"];  //支付结果异步通知
