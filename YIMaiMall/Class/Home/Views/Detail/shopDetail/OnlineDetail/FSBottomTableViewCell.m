...
- (void)setCellCanScroll:(BOOL)cellCanScroll
{
    ...
    for (LDBaseShopDetailController *VC in _viewControllers) {
        VC.vcCanScroll = cellCanScroll;
        if (!cellCanScroll) {//如果cell不能滑动，代表到了顶部，修改所有子vc的状态回到顶部
            VC.aCollectionView.contentOffset = CGPointZero;
        }
    }
}
...
- (void)setIsRefresh:(BOOL)isRefresh
{
    ...
    for (LDBaseShopDetailController *ctrl in self.viewControllers) {
