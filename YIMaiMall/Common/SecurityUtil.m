...
#define Iv          @"XB9DC5605CD3729A" //偏移量,可自行修改
#define KEY         @"XB3BFAC83087DB0BC1926D44F84CAC29" //key，可自行修改
...
+(NSString*)encryptAESData256:(NSString*)string withKey:(NSString *)key withIv:(NSString *)iv
{
    ...
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    //使用密码对nsdata进行加密
    NSData *encryptedData = @"";//[data AES256_Encrypt:key withIV:iv];
    //返回进行base64进行转码的加密字符串
    return [self encodeBase64Data:encryptedData];
}
...
+(NSString*)decryptAESData256:(NSString*)string withKey:(NSString *)key withIv:(NSString *)iv
{
    ...
    //使用密码对data进行解密
    NSData *decryData = @"";//[decodeBase64Data AES256_Decrypt:key withIV:iv];
