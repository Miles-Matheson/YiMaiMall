...
/**发送POST异步请求
 ...
 4.failedBlock  : 请求失败时回调
 */
+ (void)postWithUrl:(NSString *)urlStr dataDic:(NSDictionary *)dataDic paraDic:(NSDictionary *)paraDic successBlock:(requestSuccessBlock)successBlock failedBlock:(requestFailedBlock)failedBlock{
...
//    // 开始设置请求头
    NSString *client_type =  dataDic[@"client"];
    NSString *version    = dataDic[@"version"];
    NSString *apiVersion = dataDic[@"apiVersion"];
    NSString *timestamp = dataDic[@"timestamp"];
    NSString *postfix =  dataDic[@"postfix"];
//
    [manager.requestSerializer setValue:postfix?postfix:@"" forHTTPHeaderField:@"POSTFIX"];
    [manager.requestSerializer setValue:timestamp?timestamp:@"" forHTTPHeaderField:@"TIME"];
    [manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];;
    [manager.requestSerializer setValue:client_type?client_type:@"" forHTTPHeaderField:@"CLIENT-TYPE"];
    [manager.requestSerializer setValue:version?version:@"" forHTTPHeaderField:@"VERSION"];
    [manager.requestSerializer setValue:apiVersion?apiVersion:@"" forHTTPHeaderField:@"APIVERSION"];
    ...
    [manager POST:urlStr parameters:paraDic progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (successBlock) {
            successBlock(isEqualValue(jsonObj[@"state"], 1),jsonObj[@"msg"],jsonObj);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
